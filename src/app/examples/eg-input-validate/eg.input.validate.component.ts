...
@Component({
  selector: 'app-home',
  templateUrl: './eg.input.validate.component.html',
  styleUrls: ['./eg.input.validate.component.scss'],
})
export class ExampleInputValidateComponent implements OnInit, OnDestroy {
  ...
  onInuputChange01(event: string) {
    ...
    if (event.length > ExampleInputValidateComponent.MAX_INPUT_LENGTH) {
      this.isInputInvalid01 = true;
      this.tooltipMessage01 = 'Maximum length exceeded';

      setTimeout(() => {
        this.isInputInvalid01 = false;
        this.myInput01 = event.substring(0, ExampleInputValidateComponent.MAX_INPUT_LENGTH);
      }, 1000);
    } else {
      this.isInputInvalid01 = false;
    }
